...
export const rename_scenario_test = async ({
                                               source_path,
                                               target_path,
                                               scenario,
                                           }: {
    ...
    const sanitized_source = sanitize_scenario_path(source_path)
    const sanitized_target = sanitize_scenario_path(target_path)

    if (sanitized_source === sanitized_target)
        throw Error("Enter a different name.")
    ...
    const source_folder = get_test_folder_path(sanitized_source)
    const target_folder = get_test_folder_path(sanitized_target)
    if (source_folder !== target_folder) {
        const powers = await load_test_powers_by_path(sanitized_source)
        if (powers.powers.length > 0)
            await save_test_powers_by_path({test_path: sanitized_target, file: powers})
    }

    await delete_scenario_test(sanitized_source)
