...
export const get_reach = ({instruction, origin, battle_grid, evaluate_token}: {
        ...
        switch (instruction.targeting_type) {
            ...
            case "ranged": {
                const distance = EXPR.as_number_resolved(evaluate_token(instruction.distance))
            ...
            case "push": {
                const anchor = EXPR.as_position(evaluate_token(instruction.anchor))
                const origin = EXPR.as_position(evaluate_token(instruction.origin))
                const distance = EXPR.as_number_resolved(evaluate_token(instruction.distance)).value
