...
export class Scanner {
    ...
    get_text_while = (condition: (char: string) => boolean) => {
        ...
        while (condition(this.peek()))
            value += this.next()
        assert(value !== "", () => `While scanning ${this.text} fabricated an empty text from text_while`)
        return value
    }
    ...
    assert_peek = (expectation: string) => {
        this.assert_not_at_end()
        assert(expectation === this.peek(), () => `While scanning ${this.text} expected ${expectation} but found ${this.peek()} at index ${this.index}`)
    }

    assert_not_at_end = () => {
        assert(!this.is_at_end(), () => `While scanning ${this.text} found premature end of text`)
    }
}
